...
class ControllerREST(http.Controller):
    ...
    def api_auth_gettokens(self, **post):
        ...
        email = post['email'] if post.get('email') else None
        user_email = False
        ...
        if email:
            # Verify email if exist, for google sign in (Dashboard HOKI)
            user = request.env['res.users'].sudo().search([
                ('oauth_uid', '=', email)
            ])
            if not user:
                info = "Email {} is not registered in DMS!".format(email)
                error = 'unregistered_email'
                _logger.error(info)
                return invalid_response(400, error, info)

            username = user.login
            password = user.password_crypt
            user_email = user.oauth_uid
            uid = user.id
